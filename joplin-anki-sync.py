...
def config_parser():
    ...
    token_json = ""
    paths = (
        f'{os.getenv("HOME")}/.config/joplin-desktop/joplin-anki-sync/token.json',
    ...
    if os.path.isfile(paths[0]) or os.path.isfile(paths[1]):
        ...
        with open(path) as config_file:
            try:
                ...
                print(
                    f"[Error] JSON decoder error: {error}. Please check '{path}' syntax."
                )
                exit()
    else:
        print(
    ...
    config_json = ""
    paths = (
        f'{os.getenv("HOME")}/.config/joplin-desktop/joplin-anki-sync/config.json',
...
def joplin_note_parser(note_name, note_id):
    ...
    for header in headers:
        ...
        if "==" in header:
            continue
        write_segment = (
            None  # Is used to decide where is active content of specific H1 header
        )
...
def anki_check_and_add_decks(folders):
    anki_json = {"action": "deckNames", "version": 6}
    response = requests.post(anki_origin, json=anki_json)
    note_json = json.loads(response.content)
    for deck in folders:
        if deck in note_json["result"]:
            continue
        else:
            anki_json = {
                "action": "createDeck",
...
def statistic():
    ...
    for card in deleted:
        print(" -", card)
    syslog.syslog(
        f"Created: {len(created)}, Updated: {len(updated)}, Deleted: {len(deleted)}"
    )


config_parser()

anki_check_and_add_decks(folders)
